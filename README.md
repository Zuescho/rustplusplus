...
| `RPP_STEAM_SCRAPE_DELAY_MS` | `1500` | **Fork.** Minimum gap in ms between two teammate-avatar backfill scrapes. After a reconnect the team's avatars are refilled one profile per interval instead of all at once, so Steam stops 429-ing the burst. |
| `RPP_STEAM_AVATAR_CACHE_MS` | `21600000` (6h) | **Fork.** How long a scraped Steam avatar URL is reused. Death and login notifications scrape one avatar per event, so without this an active team asks Steam for the same faces dozens of times an hour and gets 429-ed. Set to `0` to disable the cache entirely: the background team-avatar priming is switched off too, and notifications fall back to scraping per event exactly as they did before this existed. |
